...
class Individual:
    ...
    def __init__(
        self, operators, terminals, max_depth, model_class=None, fitnessType="Accuracy"
    ):
    ...
    def __gt__(self, other):
        ...
        return (
            (sf > of) or (sf == of and sd < od) or (sf == of and sd == od and ss < os)
        )
    ...
    def getFitness(self, tr_x=None, tr_y=None):
        ...
        if self.fitness is None:
            ...
            if self.fitnessType == "MSE":
                ...
                self.getTrainingPredictions()
                mse = float(
                    -1 * mean_squared_error(self.trainingPredictions, self.training_Y)
                )
                self.fitness = mse
            ...
            if self.fitnessType == "WAF":
                ...
                self.getTrainingPredictions()
                waf = f1_score(
                    self.trainingPredictions, self.training_Y, average="weighted"
                )
                self.fitness = waf
    ...
    def calculate(self, sample):
        ...
        """
        return [
            self.dimensions[i].calculate(sample) for i in range(len(self.dimensions))
        ]
