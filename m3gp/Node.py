...
class Node:
    ...
    def calculate(self, sample):
        ...
        if self.branches == None:
            ...
            if self.value == "+":  # +
                return self.branches[0].calculate(sample) + self.branches[1].calculate(
                    sample
                )
            if self.value == "-":  # -
                return self.branches[0].calculate(sample) - self.branches[1].calculate(
    ...
    def prun(self, tr_x):
        ...
        if self.branches != None and len(self.branches) == 2:  # [+, -, *, /]
            ...
            if self.value == "+":
                # 0 + X == X
                if not self.isLeaf() and (
                    self.branches[0].isLeaf() and self.branches[0].value == "0.0"
                ):
                    self.redirect(self.branches[1])

                # X + 0 == X
                if not self.isLeaf() and (
                    self.branches[1].isLeaf() and self.branches[1].value == "0.0"
                ):
                    self.redirect(self.branches[0])

                # X + X == 2 * X
                if not self.isLeaf() and (
                    str(self.branches[1]) == str(self.branches[0])
                ):
                    self.value = "*"
                    n = Node()
                    n.copy(value="2.0")
            ...
            if self.value == "*":
                ...
                if not self.isLeaf() and (
                    (self.branches[0].isLeaf() and self.branches[0].value == "0.0")
                    or (self.branches[1].isLeaf() and self.branches[1].value == "0.0")
                ):
                    n = Node()
                    n.copy(value="0.0")
                    self.redirect(n)
                ...
                # 1 * X == X
                if not self.isLeaf() and (
                    self.branches[0].isLeaf() and self.branches[0].value == "1.0"
                ):
                    self.redirect(self.branches[1])

                # X * 1 == X
                if not self.isLeaf() and (
                    self.branches[1].isLeaf() and self.branches[1].value == "1.0"
                ):
                    self.redirect(self.branches[0])
