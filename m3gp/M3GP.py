...
class M3GP:
    ...
    def __init__(
        ...
        if (
            ...
            print(
                "[Warning] Some of the following operators may not be supported:",
                operators,
            )
        ...
        while self.currentGeneration < self.max_generation:
            ...
            if not self.Te_x is None:
                if self.fitnessType in ["Accuracy", "2FOLD", "WAF"]:
                    self.trainingAccuracyOverTime.append(
                        self.bestIndividual.getAccuracy(self.Tr_x, self.Tr_y, pred="Tr")
                    )
                    self.testAccuracyOverTime.append(
                        self.bestIndividual.getAccuracy(self.Te_x, self.Te_y, pred="Te")
                    )
                    self.trainingWaFOverTime.append(
                        self.bestIndividual.getWaF(self.Tr_x, self.Tr_y, pred="Tr")
                    )
                    self.testWaFOverTime.append(
                        self.bestIndividual.getWaF(self.Te_x, self.Te_y, pred="Te")
                    )
                    self.trainingKappaOverTime.append(
                        self.bestIndividual.getKappa(self.Tr_x, self.Tr_y, pred="Tr")
                    )
                    self.testKappaOverTime.append(
                        self.bestIndividual.getKappa(self.Te_x, self.Te_y, pred="Te")
                    )
                    ...
                    self.trainingMSEOverTime.append(
                        self.bestIndividual.getMSE(self.Tr_x, self.Tr_y, pred="Tr")
                    )
                    self.testMSEOverTime.append(
                        self.bestIndividual.getMSE(self.Te_x, self.Te_y, pred="Te")
                    )
                self.sizeOverTime.append(self.bestIndividual.getSize())
                self.dimensionsOverTime.append(
                    self.bestIndividual.getNumberOfDimensions()
                )
    ...
    def nextGeneration(self):
        ...
        if self.threads > 1:
            with mp.Pool(processes=self.threads) as pool:
                results = pool.map(
                    fitIndividuals,
                    [(ind, self.Tr_x, self.Tr_y) for ind in self.population],
                )
        ...
        if self.verbose and self.currentGeneration % 5 == 0:
            if not self.Te_x is None:
                print(
                    "   > Gen #%2d:  Fitness: %.6f // Tr-Score: %.6f // Te-Score: %.6f  // Time: %.4f"
                    % (
                        self.currentGeneration,
                        self.bestIndividual.getFitness(),
                        self.bestIndividual.getTrainingMeasure(),
                        self.bestIndividual.getTestMeasure(self.Te_x, self.Te_y),
                        end - begin,
                    )
                )
            else:
                print(
                    "   > Gen #%2d:  Fitness: %.6f // Tr-Score: %.6f // Time: %.4f"
                    % (
                        self.currentGeneration,
                        self.bestIndividual.getFitness(),
                        self.bestIndividual.getTrainingMeasure(),
    ...
    def predict(self, dataset):
        ...
        return (
            ...
            else self.bestIndividual.predict(sample)
        )
