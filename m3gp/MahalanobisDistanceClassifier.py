...
class MahalanobisDistanceClassifier:
    ...
    def fit(self, X, Y):
        ...
        for cluster in clusters:
            ...
            for sample in cluster:
                self.classCentroids[-1] = [
                    self.classCentroids[-1][i] + sample[i] / len(cluster)
                    for i in range(len(self.classCentroids[-1]))
                ]
    ...
    def predict(self, X):
        ...
        for sample in X:
            pick_d = mahalanobisDistance(
                sample, self.classCentroids[0], self.invCovarianceMatrix[0]
            )
            pick = self.classes[0]

            for i in range(len(self.classes)):
                d = mahalanobisDistance(
                    sample, self.classCentroids[i], self.invCovarianceMatrix[i]
                )
                if d < pick_d:
                    pick_d = d
                    pick = self.classes[i]
